<template>
  <div class="app-container">
    ...
    >
      ...
      <el-form-item label="部门：" prop="sourceLocationId" label-width="120px">
        <el-select
      ...
      <el-tab-pane label="退库单据明细">
        ...
        <el-form :model="form" :rules="tableRules" ref="formRef">
          ...
          >
            ...
            <!-- <el-table-column label="国家医保码" align="center" key="ybNo" prop="ybNo" width="130">
              <template #default="scope">
                <el-form-item
                  ...
                  disabled
                >
                  <el-input v-model="scope.row.ybNo" placeholder="" />
                </el-form-item>
              </template>
            </el-table-column> -->
            <el-table-column
...
const data = reactive({
  ...
  rules: {
    purposeLocationId: [{ required: true, message: '请选择部门', trigger: 'change' }],
    medicationType: [{ required: true, message: '请选择药品类型', trigger: 'change' }],
    practitionerId: [{ required: true, message: '请选择部门经手人', trigger: 'change' }],
  },
  tableRules: {
    purposeLocationId: [{ required: true, message: '仓库不能为空', trigger: 'change' }],
    ...
    itemQuantity: [{ required: true, message: '退库数量不能为空', trigger: 'blur' }],
    // lotNumber: [
...
function handleBlur(row, index) {
  ...
  let sourceLocation = issueDepartmentDto.value.filter((e) => {
    return (e.locationId == receiptHeaderForm.sourceLocationId||e.locationId == receiptHeaderForm.sourceLocationId1)
  });
  console.log('sourceLocation',issueDepartmentDto,receiptHeaderForm.sourceLocationId,sourceLocation);
  // 部门
  ...
  if (route.query.supplyBusNo) {
    getTHTransferProductDetail(receiptHeaderForm.busNo).then((res) => {
      ...
      form.purchaseinventoryList.map((e, index) => {
        ...
        console.log(e,e.unitList&&e.unitCode,!e.unitList&&e.unitCode,12121212)
...
function saveRow(row, index) {
  ...
  addTHTransferProduct(row).then((response) => {
    reset();
...
function handleReject() {
  reject(route.query.supplyBusNo).then((res) => {
    if (res.code == 200) {
      ...
      // 跳转到审核页面
      router.replace({ path: '/aaaa/medicationmanagement/billapproval',query:{type:'returningInventory'}});
    }
  });
}
...
function toLastView(visitedViews, view) {
  ...
  if(view.name== 'ReturningInventory'){ //调拨单据号删除
...
function selectRow(rowValue, index) {
  ...
  form.purchaseinventoryList[index].totalPrice = 0
  store.setCurrentDataLYTK({ purchaseinventoryList: form.purchaseinventoryList, receiptHeaderForm: receiptHeaderForm });
}
...
function handleLocationClick(item, row, index) {
  getCount({
    itemId: row.itemId,
    orgLocationId: form.purchaseinventoryList[index].purposeLocationId,
    ...
    if (res.data && res.data.length > 0) {
      ...
      // 仓库类型
      form.purchaseinventoryList[index].purposeTypeEnum = item.formEnum;
      ...
      if(form.purchaseinventoryList[index].unitCode==form.purchaseinventoryList[index].unitList.minUnitCode){
        ...
        console.log(form.purchaseinventoryList[index].price,"!21221")
        // if(form.purchaseinventoryList[index].price>1){
        //   form.purchaseinventoryList[index].price = form.purchaseinventoryList[index].price.toFixed(4)
        // }
      }
...
//       row.totalSourceQuantity / row.partPercent;
//     form.purchaseinventoryList[index].totalPurposeQuantity =
//       row.totalPurposeQuantity / row.partPercent;
//     // 数量
...
function deleteSelectedRows() {
  ...
  if (selectedRows.value[length - 1].isSave) {
    delTHTransferProduct(ids).then((res) => {
      if (res.code == 200) {
...
function edit() {
  ...
  receiptHeaderForm.occurrenceTime = editData.value.editRow.occurrenceTime?formatDate(editData.value.editRow.occurrenceTime):formatDate(new Date())
  receiptHeaderForm.purposeTypeEnum = props.editRow.purposeTypeEnum.toString();
  receiptHeaderForm.sourceTypeEnum = props.editRow.purposeTypeEnum.toString();
  ...
  handleChangeSourceTypeEnum(props.editRow.sourceTypeEnum.toString());
  setTimeout(() => {
...
function getBusNoInitList() {
  if (route.query.supplyBusNo) {
    ...
    if (!sessionStorage.getItem('busNoLYTH')) {
      store.clearCurrentDataLYTK()
      getTHBusNoInit().then((response) => {
        receiptHeaderForm.busNo = response.data.busNo;
        sessionStorage.setItem('busNoLYTH', receiptHeaderForm.busNo);
        // busNoAdd.value = response.data.busNo; // 单据号新增
      });
    } else {
      receiptHeaderForm.busNo = sessionStorage.getItem('busNoLYTH');
    }
  }
}
