...
import com.core.flowable.domain.dto.FlowCommentDto;
import com.core.flowable.domain.dto.FlowNextDto;
import com.core.flowable.domain.dto.FlowTaskDto;
import com.core.flowable.domain.dto.FlowViewerDto;
...
import com.core.flowable.flow.CustomProcessDiagramGenerator;
import com.core.flowable.flow.FindNextNodeUtil;
import com.core.flowable.flow.FlowableUtils;
...
public class FlowTaskServiceImpl extends FlowServiceFactory implements IFlowTaskService {
    ...
    public void taskReject(FlowTaskVo flowTaskVo) {
        ...
        List<String> lastHistoricTaskInstanceList =
            FlowableUtils.historicTaskInstanceClean(allElements, historicTaskInstanceList);
        ...
        List<UserTask> currentUserTaskList =
            FlowableUtils.iteratorFindChildUserTasks(oneUserTask, runTaskKeyList, null, null);
    ...
    public void taskReturn(FlowTaskVo flowTaskVo) {
        ...
        Boolean isSequential =
            FlowableUtils.iteratorCheckSequentialReferTarget(source, flowTaskVo.getTargetKey(), null, null);
        if (!isSequential) {
    ...
    /**
     * 认领/签收任务 认领以后,这个用户就会成为任务的执行人,任务会从其他成员的任务列表中消失
     *
     * @param flowTaskVo 请求实体参数
     */
    @Override
    ...
    /**
     * 委派任务 任务委派只是委派人将当前的任务交给被委派人进行审批，处理任务后又重新回到委派人身上。
     *
     * @param flowTaskVo 请求实体参数
     */
    @Override
    ...
    /**
     * 转办任务 直接将办理人换成别人，这时任务的拥有者不再是转办人
     *
     * @param flowTaskVo 请求实体参数
     */
    @Override
    ...
    /**
     * 多实例加签 act_ru_task、act_ru_identitylink各生成一条记录
     *
     * @param flowTaskVo
     */
    @Override
    ...
    /**
     * 多实例减签 act_ru_task减1、act_ru_identitylink不变
     *
     * @param flowTaskVo
     */
    @Override
    ...
    public AjaxResult myProcess(FlowQueryVo queryVo) {
        ...
        HistoricProcessInstanceQuery historicProcessInstanceQuery = historyService.createHistoricProcessInstanceQuery()
            .startedBy(userId.toString()).orderByProcessInstanceStartTime().desc();
        ...
        for (HistoricProcessInstance hisIns : historicProcessInstances) {
            ...
            if (CollectionUtils.isNotEmpty(taskList)) {
                ...
                if (StringUtils.isNotBlank(historicTaskInstance.get(0).getAssignee())) {
                    // 当前任务节点办理人信息
                    SysUser sysUser =
                        sysUserService.selectUserById(Long.parseLong(historicTaskInstance.get(0).getAssignee()));
                    if (Objects.nonNull(sysUser)) {
                        flowTask.setAssigneeId(sysUser.getUserId());
                        flowTask.setAssigneeName(sysUser.getNickName());
                        flowTask.setAssigneeDeptName(
    ...
    public AjaxResult revokeProcess(FlowTaskVo flowTaskVo) {
        ...
        // 跳转节点
        runtimeService.createChangeActivityStateBuilder().processInstanceId(flowTaskVo.getInstanceId())
            .moveActivityIdTo(task.getTaskDefinitionKey(), tarKey).changeState();
    ...
    public AjaxResult todoList(FlowQueryVo queryVo) {
        ...
        List<Task> taskList =
            taskQuery.listPage(queryVo.getPageSize() * (queryVo.getPageNum() - 1), queryVo.getPageSize());
        List<FlowTaskDto> flowList = new ArrayList<>();
    ...
    public AjaxResult finishedList(FlowQueryVo queryVo) {
        ...
        HistoricTaskInstanceQuery taskInstanceQuery =
            historyService.createHistoricTaskInstanceQuery().includeProcessVariables().finished()
                .taskAssignee(userId.toString()).orderByHistoricTaskInstanceEndTime().desc();
        List<HistoricTaskInstance> historicTaskInstanceList =
            taskInstanceQuery.listPage(queryVo.getPageSize() * (queryVo.getPageNum() - 1), queryVo.getPageSize());
    ...
    public AjaxResult flowRecord(String procInsId, String deployId) {
        ...
        if (StringUtils.isNotBlank(procInsId)) {
            ...
            for (HistoricActivityInstance histIns : list) {
                ...
                // FlowTaskDto flowTask = new FlowTaskDto();
                // flowTask.setTaskName(StringUtils.isNotBlank(histIns.getActivityName()) ? histIns.getActivityName() :
                // "结束");
                // flowTask.setFinishTime(histIns.getEndTime());
                // hisFlowList.add(flowTask);
                // } else
                if (StringUtils.isNotBlank(histIns.getTaskId())) {
                    ...
                    List<HistoricIdentityLink> linksForTask =
                        historyService.getHistoricIdentityLinksForTask(histIns.getTaskId());
                    ...
                    for (HistoricIdentityLink identityLink : linksForTask) {
                        ...
                        if ("candidate".equals(identityLink.getType())) {
                            if (StringUtils.isNotBlank(identityLink.getUserId())) {
                                SysUser sysUser =
                                    sysUserService.selectUserById(Long.parseLong(identityLink.getUserId()));
                                stringBuilder.append(sysUser.getNickName()).append(",");
                            }
                            if (StringUtils.isNotBlank(identityLink.getGroupId())) {
                                SysRole sysRole =
                                    sysRoleService.selectRoleById(Long.parseLong(identityLink.getGroupId()));
                    ...
                    flowTask.setDuration(histIns.getDurationInMillis() == null || histIns.getDurationInMillis() == 0
                        ? null : getDate(histIns.getDurationInMillis()));
                    // 获取意见评论内容
    ...
    public AjaxResult getFlowViewer(String procInsId, String executionId) {
        ...
        List<HistoricActivityInstance> startNodeList = historyService.createHistoricActivityInstanceQuery()
            .processInstanceId(procInsId).orderByHistoricActivityInstanceStartTime().asc().listPage(0, 3);
    ...
    public AjaxResult getNextFlowNodeByStart(FlowTaskVo flowTaskVo) {
        ...
        ProcessDefinition processDefinition =
            repositoryService.createProcessDefinitionQuery().deploymentId(flowTaskVo.getDeploymentId()).singleResult();
        if (Objects.isNull(processDefinition)) {
            return AjaxResult.error("流程信息不存在!");
        }
        // Step 2. 获取下一任务节点(网关节点时需要校验表达式)
        List<UserTask> nextUserTask =
            FindNextNodeUtil.getNextUserTasksByStart(repositoryService, processDefinition, flowTaskVo.getVariables());
        if (CollectionUtils.isEmpty(nextUserTask)) {
    ...
    private AjaxResult getFlowAttribute(List<UserTask> nextUserTask) {
        ...
        for (UserTask userTask : nextUserTask) {
            ...
            if (Objects.nonNull(multiInstance)) {
                ...
                String userType =
                    userTask.getAttributeValue(ProcessConstants.NAMASPASE, ProcessConstants.PROCESS_CUSTOM_USER_TYPE);
    ...
    public AjaxResult flowXmlAndNode(String procInsId, String deployId) {
        try {
            List<FlowViewerDto> flowViewerList = new ArrayList<>();
            // 获取已经完成的节点
            List<HistoricActivityInstance> listFinished =
                historyService.createHistoricActivityInstanceQuery().processInstanceId(procInsId).finished().list();
            ...
            // 获取代办节点
            List<HistoricActivityInstance> listUnFinished =
                historyService.createHistoricActivityInstanceQuery().processInstanceId(procInsId).unfinished().list();
    ...
    public AjaxResult flowTaskInfo(String procInsId, String elementId) {
        List<HistoricActivityInstance> list = historyService.createHistoricActivityInstanceQuery()
            .processInstanceId(procInsId).activityId(elementId).list();
