...
public class FlowableUtils {
            ...
            for (SequenceFlow sequenceFlow : sequenceFlows) {
                ...
                if (sequenceFlow.getSourceFlowElement() instanceof SubProcess) {
                    ...
                    List<UserTask> childUserTaskList = findChildProcessUserTasks(
                        (StartEvent)((SubProcess)sequenceFlow.getSourceFlowElement()).getFlowElements().toArray()[0],
                        null, null);
            ...
            for (SequenceFlow sequenceFlow : sequenceFlows) {
                ...
                if (sequenceFlow.getTargetFlowElement() instanceof SubProcess) {
                    List<UserTask> childUserTaskList = iteratorFindChildUserTasks(
                        (FlowElement)(((SubProcess)sequenceFlow.getTargetFlowElement()).getFlowElements().toArray()[0]),
    ...
    public static Boolean iteratorCheckSequentialReferTarget(FlowElement source, String targetKsy,
        ...
        if (source instanceof StartEvent && source.getSubProcess() != null) {
            isSequential =
                iteratorCheckSequentialReferTarget(source.getSubProcess(), targetKsy, hasSequenceFlow, isSequential);
        }
    ...
    public static List<Object> getPropertyElement(RepositoryService repositoryService,
        ...
        ExtensionElement extensionElement =
            FlowableUtils.getExtensionElementFromFlowElementByName(flowElement, "properties");
        ...
        try {
            ...
            return Optional.ofNullable(extensionElement.getChildElements().get(attributesName))
                ...
                .stream().map(element -> {
                    ...
                    Arrays.stream(propertyDto.getClass().getDeclaredFields()).forEach(field -> {
                        field.setAccessible(true);
                        // 获取属性名称和值
                        attributes.getOrDefault(field.getName(), Collections.emptyList()).stream().findFirst()
                            .ifPresent(attribute -> {
                                try {
                                    // 反射设置属性值
