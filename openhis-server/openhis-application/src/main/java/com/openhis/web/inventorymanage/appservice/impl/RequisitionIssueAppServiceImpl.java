 ...
 */
@Service
public class RequisitionIssueAppServiceImpl implements IRequisitionIssueAppService {

    @Autowired
    private RequisitionIssueMapper requisitionIssueMapper;
    ...
    public R<?> requisitionIssueNoInit() {
        IssueInitDto initDto = new IssueInitDto();
        // 单据号
        initDto.setBusNo(assignSeqUtil.getSeqByDay(AssignSeqEnum.REQUISITION_NUM.getPrefix(), 10));
        return R.ok(initDto);
    }
    ...
    public R<?> getDetail(String busNo) {
        ...
        for (IssueDetailDto dto : requisitionIssueDetailList) {
        ...
        return R.ok(requisitionIssueDetailList);
    }

    /**
     * 添加/编辑领用出库单据(批量)
     *
     * @param requisitionIssueDtoList 领用出库单据
     * @return 操作结果
     */
    @Override
    public R<?> addOrEditIssueReceipt(List<IssueDto> requisitionIssueDtoList) {

        List<String> idList = new ArrayList<>();

        // 单据号取得
        List<String> busNoList =
                requisitionIssueDtoList.stream().map(IssueDto::getBusNo).collect(Collectors.toList());
        // 请求数据取得
